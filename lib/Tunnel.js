  ...
  _init(cb) {
    ...
    function getUrl() {
      return axios
        ...
        .catch(err => {
          if (err.errno === -111) {
            console.error(`\u001b[31mRequest failed to connect ${err.code}`);
            console.error(`---- URI ${err.config.url}\u001b[0m`)
            process.exit(1);
          }
          if (parseInt(err.response.status)  > 399) {
            //If the server response is a 4XX error, we print and exit
            console.error(`\u001b[31mRequest to the Tunneling server failed (${err.response.status})`);
            console.error(`---- URI ${err.config.url}`)
            console.error(`---- Data ${JSON.stringify(err.response.data)}\u001b[0m`)
            process.exit(1);
          }
